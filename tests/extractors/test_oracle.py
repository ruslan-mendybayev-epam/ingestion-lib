...
class TestOracleExtractor(unittest.TestCase):
    ...
    def test_condition_returns_empty_string(self):
        ...
        self.extractor.data_contract = MagicMock(spec=TableContract, watermark_columns=[""], full_load=True, load_type="one_time")
        self.assertEqual(self.extractor._Extractor__build_condition(), "")

        # Scenario 2: full_load is "true"
        self.extractor.data_contract = MagicMock(spec=TableContract, watermark_columns=["timestamp"], full_load="true", load_type="incremental")
        self.assertEqual(self.extractor._Extractor__build_condition(), "")

        # Scenario 3: load_type is "one_time"
        self.extractor.data_contract = MagicMock(spec=TableContract, watermark_columns=["timestamp"], full_load="false", load_type="one_time")
        self.assertEqual(self.extractor._Extractor__build_condition(), "")
    def test_build_select_query(self):
        # Test scenarios for SQL query construction

        # Scenario 1: Basic select query
        self.extractor.data_contract = MagicMock(spec=TableContract, table_name="test_table", schema_name="test_schema", watermark_columns=None, full_load="false")
